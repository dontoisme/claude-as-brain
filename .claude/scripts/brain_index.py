...
The `retrievals` table and activation formula are in place for Phase B, which
adds the bump logic; until rows exist activation is neutral (1.0).
...
def load_config(vault: Path) -> dict:
    ...
    cfg: dict = {"half_life_days": dict(DEFAULT_HALF_LIFE), "quarantine": []}
...
def activation(conn: sqlite3.Connection, rel: str, now: dt.datetime) -> tuple[float, int, dt.datetime | None]:
    ...
    rows = conn.execute("SELECT ts FROM retrievals WHERE path=?", (rel,)).fetchall()
    if not rows:
        return 1.0, 0, None
    last = None
    total = 0.0
...
def cmd_rank(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        return 0
    print(f"terms: {', '.join(terms)} · profile: {args.profile} · {len(cands)} candidate(s)")
    for r in results:
...
def main(argv: list[str] | None = None) -> int:
    ...
    k.add_argument("--profile", choices=sorted(PROFILE_MULT), default="current",
                   help="intent profile (Phase C classifies; Phase A callers pass it explicitly or leave the default)")
